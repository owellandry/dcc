    ...
    let (rows, reverse_after) = if let Some(before) = q.before {
        let r = sqlx::query_as::<_, MessageRow>(
            r#"SELECT m.id, m.channel_id, m.author_id, m.content, m.message_type,
                      ...
                      u.avatar_url as author_avatar_url, u.avatar_decoration_url as author_avatar_decoration_url, u.banner_url as author_banner_url,
...
async fn fetch_reply_preview(state: &AppState, reply_to_id: Option<Uuid>) -> Result<Option<Value>> {
    ...
    struct ReplyPreviewRow {
        ...
        author_avatar_decoration_url: Option<String>,
    ...
    let reply = sqlx::query_as::<_, ReplyPreviewRow>(
        r#"SELECT m.id, m.content,
                  ...
                  u.avatar_decoration_url as author_avatar_decoration_url,
    ...
    Ok(reply.map(|row| {
        json!({
            ...
            "author": {
                ...
                "avatarDecorationUrl": row.author_avatar_decoration_url,
