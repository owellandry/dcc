...
export const MOCK_CHANNELS: Channel[] = [
  ...
  },
  {
    ...
    categoryId: 'cat-2',
    name: 'devops',
    topic: 'Docker, CF Workers, deploy',
    type: 'text',
    position: 2,
    isNsfw: false,
    slowmodeSeconds: 0,
...
export const MOCK_MESSAGES: Record<string, Message[]> = {
  'ch-general': [
    ...
    msg(
      ...
      'ch-general',
      1,
      'honestly loving it. the tower middleware composability is 🔥',
      113,
      {
        reactions: [
          { emoji: '🔥', count: 3, meReacted: true },
          { emoji: '💯', count: 2, meReacted: false },
        ],
      }
    ),
    msg('m7', 'ch-general', 3, "meanwhile I'm fighting tailwind specificity", 110),
    msg('m8', 'ch-general', 0, 'haha skill issue', 109, {
      reactions: [{ emoji: '💀', count: 4, meReacted: true }],
    }),
    ...
    msg(
      ...
      'ch-general',
      0,
      'yeah just gotta be careful with node APIs. no fs, no crypto directly',
      86
    ),
    ...
    msg(
      ...
      'ch-general',
      3,
      'btw I pushed the new design system tokens. check #tokens in the design server',
      60
    ),
    msg('m17', 'ch-general', 0, 'saw it! the ember + volt combo looks fire', 58),
    msg('m18', 'ch-general', 1, 'agreed. way better than the old generic purple', 57, {
      reactions: [{ emoji: '❤️', count: 2, meReacted: false }],
    }),
  ...
  'ch-frontend': [
    ...
    msg(
      ...
      'ch-frontend',
      0,
      'we load 50 messages at a time. when you scroll to the top it fetches older ones and prepends them',
      196
    ),
    ...
    msg(
      ...
      'ch-frontend',
      0,
      'message gets appended to the store immediately, then the WS event deduplicates it when it arrives',
      178
    ),
    msg('m-f8', 'ch-frontend', 3, 'smart. so no double messages', 177),
    msg('m-f9', 'ch-frontend', 0, 'exactly 👌', 176, {
      reactions: [{ emoji: '👌', count: 1, meReacted: false }],
    }),
  ],
  'ch-backend': [
    msg(
      ...
      'ch-backend',
      1,
      'sqlx is incredible btw. compile-time query checking is game changing',
      300
    ),
    ...
    msg(
      ...
      'ch-backend',
      0,
      'nice. make sure the index is on (channel_id, created_at DESC)',
      292
    ),
    msg('m-b6', 'ch-backend', 1, 'already on it. EXPLAIN ANALYZE shows index-only scan 🎉', 290, {
      reactions: [{ emoji: '🎉', count: 2, meReacted: true }],
    }),
  ],
  'ch-welcome': [
    msg(
      ...
      'ch-welcome',
      0,
      '# Welcome to DCC Dev Hub! 👋\n\nThis is where we build DCC and ship features fast.\n\nCheck out the channels below to get started.',
      10000
    ),
  ],
  'ch-memes': [
    msg('m-mm1', 'ch-memes', 2, 'me writing TypeScript vs me writing Rust', 500),
    msg('m-mm2', 'ch-memes', 1, 'borrow checker jumpscare', 499, {
      reactions: [
        { emoji: '💀', count: 5, meReacted: true },
        { emoji: '😂', count: 3, meReacted: false },
      ],
    }),
    msg(
      'm-mm3',
      'ch-memes',
      3,
      'the css is not doing what I want it to do meme but its tailwind',
      490
    ),
...
export const MOCK_PRESENCE: Record<
  string,
  { status: User['status']; customStatus: string | null }
> = {
