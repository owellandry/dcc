...
export function MediaSourcePickerModal({
  ...
  useEffect(() => {
    ...
    if (!GIPHY_API_KEY) {
      setResults([])
      setHelperText(
        'Configura NEXT_PUBLIC_GIPHY_API_KEY para habilitar la busqueda integrada de Giphy.'
      )
      return
    }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-[var(--b1)] px-4 py-3">
        <p className="font-display font-700 text-[17px] text-[var(--t0)]">{title}</p>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="space-y-4 p-4">
        <button
          ...
          disabled={isBusy || isSubmitting}
        >
          <span className="font-700 flex items-center gap-2 text-sm text-[var(--t1)]">
            <FolderUp size={16} />
            Desde tu PC
          </span>
        ...
        <div className="rounded-xl border border-[var(--b1)] bg-[var(--s2)] p-3">
          <p className="font-700 mb-2 flex items-center gap-2 text-sm text-[var(--t1)]">
            <Sparkles size={15} />
            Buscar en Giphy
          </p>
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            ...
            <span className="font-700 text-[10px] uppercase tracking-[0.14em] text-[var(--t4)]">
              Powered by GIPHY
            </span>
          </div>
                ...
                {results.map((gif) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent px-2 py-2">
                      <p className="font-700 line-clamp-2 text-[11px] leading-tight text-white">
                        {gif.title || 'GIF'}
                      </p>
                    </div>
