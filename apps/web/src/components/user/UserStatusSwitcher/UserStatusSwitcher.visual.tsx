  ...
  return (
    <div className="relative min-w-0 flex-1" ref={statusMenuRef}>
      <div className="flex min-w-0 items-center gap-2">
        <p className="truncate text-[15px] font-700 leading-none text-[var(--t0)]">{user.username}</p>
        {hasOfficialMemberBadge({ user }) && <OfficialMemberTag compact className="-translate-y-px" />}
      </div>
      <motion.button
        type="button"
        onClick={onToggleMenu}
        className="mt-1 truncate text-[12px] capitalize leading-none text-[var(--t3)] transition-colors hover:text-[var(--t1)]"
