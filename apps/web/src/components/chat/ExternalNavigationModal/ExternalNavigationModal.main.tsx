...
export function ExternalNavigationModal({ open, url, onCancel, onContinue }: Props) {
  return (
    <AnimatePresence>
      {open && (
        <motion.div
          className="fixed inset-0 z-[160] flex items-center justify-center bg-[var(--modal-scrim)]"
          onClick={onCancel}
          initial="hidden"
          animate="visible"
          exit="exit"
        ...
        >
          <motion.div
            className="flex h-[min(92vh,680px)] w-full max-w-4xl overflow-hidden rounded-3xl border border-[var(--b1)] bg-[var(--s3)] shadow-[var(--panel-shadow)]"
          ...
          >
            <div className="hidden w-[320px] shrink-0 border-r border-[var(--b1)] bg-[linear-gradient(180deg,color-mix(in_srgb,var(--ember)_12%,var(--s1)_88%),var(--s1))] p-6 lg:flex lg:flex-col">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-display text-[18px] font-700 text-[var(--t0)]">Navegacion externa</p>
                  <p className="text-sm text-[var(--t3)]">Confirmacion de seguridad</p>
                </div>
              </div>

              <div className="mt-8 space-y-4">
                <div className="rounded-2xl border border-[var(--b1)] bg-[var(--s0)]/50 p-4">
                  <p className="text-xs font-700 uppercase tracking-[0.18em] text-[var(--t3)]">Importante</p>
                  <p className="mt-2 text-sm leading-6 text-[var(--t2)]">
                    Estas a punto de abrir un enlace fuera de la app. Revisa el dominio antes de continuar.
                  </p>
                </div>

                <div className="rounded-2xl border border-[var(--b1)] bg-[var(--s0)]/40 p-4">
                  <p className="text-xs font-700 uppercase tracking-[0.18em] text-[var(--t3)]">Consejo</p>
                  <p className="mt-2 text-sm leading-6 text-[var(--t2)]">
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              <div className="flex items-center justify-between border-b border-[var(--b1)] px-5 py-4 sm:px-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-display text-[18px] font-700 text-[var(--t0)]">Vas a salir de la app</p>
                    <p className="mt-1 text-sm text-[var(--t3)]">Confirma antes de abrir el enlace externo.</p>
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="scrollable flex-1 p-5 sm:p-6">
                <div className="mx-auto flex h-full w-full max-w-4xl flex-col justify-center">
                  <div className="rounded-[28px] border border-[var(--b1)] bg-[linear-gradient(180deg,color-mix(in_srgb,var(--s2)_92%,white_8%),var(--s1))] p-6 sm:p-8">
                    <p className="text-xs font-700 uppercase tracking-[0.22em] text-[var(--t3)]">Enlace externo</p>
                    <h2 className="mt-3 font-display text-3xl font-800 tracking-tight text-[var(--t0)] sm:text-4xl">
                      Revisa este destino antes de continuar
                    </h2>
                    <p className="mt-4 max-w-2xl text-sm leading-7 text-[var(--t2)] sm:text-[15px]">
                      Por seguridad, DCC te avisa cuando un enlace intenta abrir una pagina fuera de la aplicacion.
                      Si confias en el sitio, puedes continuar.
                    </p>

                    {url && (
                      <div className="mt-6 rounded-2xl border border-[var(--b1)] bg-[var(--s0)] px-4 py-4">
                        <p className="mb-2 text-xs font-700 uppercase tracking-[0.18em] text-[var(--t3)]">Destino</p>
              ...
              <div className="flex flex-col-reverse gap-3 border-t border-[var(--b1)] px-5 py-4 sm:flex-row sm:items-center sm:justify-end sm:px-6">
                <button
                  type="button"
                  onClick={onCancel}
                  className="rounded-xl border border-[var(--b1)] px-4 py-2.5 text-sm font-600 text-[var(--t2)] transition-colors hover:border-[var(--b2)] hover:bg-[var(--s2)] hover:text-[var(--t1)]"
                >
                  Quedarme aqui
                </button>
                <button
                  type="button"
                  onClick={onContinue}
                  className="rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-700 text-[#1f1400] transition-colors hover:bg-amber-400"
                >
                  Continuar al enlace
                </button>
              </div>
